...
<i>Это заметка о паттерне на графике, не готовая сделка: вход, стоп и объём решаешь сам. Тег <code>{tag}</code> — чтобы отфильтровать такие алерты в истории чата.</i>
""".strip()
